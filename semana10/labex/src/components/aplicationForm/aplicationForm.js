...
import { BackgroundImgForm, BotoesForm, ContainerForm, Form } from "./styled";
...
const AplicationForm = () => {
    ...
    const sendForm = (ev) => {
        ...
        const url = `https://us-central1-labenu-apis.cloudfunctions.net/labeX/${aluno}/trips/${form.id}/apply`
        const body = {
            name: form.name,
            age: form.age,
    ...
    const renderListName = list.map((trip) => {
        return (<option 
            key={trip.id} 
            value={trip.id}>
                        {trip.name}
    ...
    return (
        <ContainerForm>
                <h1>Formulário de embarque</h1>
            <div>
                <BackgroundImgForm src={backgroundForm} alt="background"/>
